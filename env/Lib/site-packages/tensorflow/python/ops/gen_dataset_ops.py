...
def model_dataset(input_dataset, output_types, output_shapes, cpu_budget=0, name=None):
  ...
  if _ctx is not None and _ctx._thread_local_data.is_eager:
    try:
      _result = _pywrap_tensorflow.TFE_Py_FastPathExecute(
        ...
        "cpu_budget", cpu_budget, "output_types", output_types,
        "output_shapes", output_shapes)
      return _result
    except _core._FallbackException:
      try:
        return model_dataset_eager_fallback(
            input_dataset, cpu_budget=cpu_budget, output_types=output_types,
            output_shapes=output_shapes, name=name, ctx=_ctx)
      except _core._SymbolicException:
        pass  # Add nodes to the TensorFlow graph.
    except _core._NotOkStatusException as e:
  ...
  _, _, _op = _op_def_lib._apply_op_helper(
        "ModelDataset", input_dataset=input_dataset,
                        output_types=output_types,
                        output_shapes=output_shapes, cpu_budget=cpu_budget,
                        name=name)
  _result = _op.outputs[:]
  _inputs_flat = _op.inputs
  _attrs = ("cpu_budget", _op.get_attr("cpu_budget"), "output_types",
            _op.get_attr("output_types"), "output_shapes",
            _op.get_attr("output_shapes"))
  _execute.record_gradient(
...
def ModelDataset(input_dataset, output_types, output_shapes, cpu_budget=0, name=None):
  return model_dataset(input_dataset=input_dataset, output_types=output_types, output_shapes=output_shapes, cpu_budget=cpu_budget, name=name)
...
def model_dataset_eager_fallback(input_dataset, output_types, output_shapes, cpu_budget=0, name=None, ctx=None):
  r"""This is the slowpath function for Eager mode.
  ...
  input_dataset = _ops.convert_to_tensor(input_dataset, _dtypes.variant)
  _inputs_flat = [input_dataset]
  _attrs = ("cpu_budget", cpu_budget, "output_types", output_types,
  "output_shapes", output_shapes)
